...
public partial class FrmAdressen : Form
{
    ...
    private static readonly string[] _eMailDomainsStandard =
    [
        "gmail.com", "gmx.de", "gmx.net", "gmx.at", "gmx.ch", "web.de", "t-online.de", "freenet.de", "yahoo.de", "yahoo.com", "hotmail.com", "hotmail.de",
        "outlook.com", "outlook.de", "live.de", "live.com", "icloud.com", "me.com", "mac.com", "posteo.de", "mailbox.org", "protonmail.com", "proton.me",
        "tutanota.com", "tutanota.de", "aon.at", "chello.at", "a1.net", "magnet.at", "bluewin.ch", "sunrise.ch", "arcor.de", "vodafone.de", "1und1.de", "msn.com",
    ];
    private string[] _eMailDomains = _eMailDomainsStandard; // wird nach dem Laden durch BuildEmailDomains() aus echten Daten befüllt
    ...
    private readonly string[] formats = ["dd.MM.yyyy", "d.MM.yyyy", "dd.M.yyyy", "d.M.yyyy", "dd.M.yy", "d.MM.yy", "d.M.yy", "d. MMMM yyyy", "dd. MMMM yyyy"];
    ...
    private bool _maskedTbFocusClick = false;       // true: nächster MouseDown ist der erste Klick nach Fokuserhalt → Sektion selektieren
    ...
    private bool _lastFocusWasMouse = false; // true wenn zuletzt per Mausklick fokussiert
    ...
    private AdvancedSearchCriteria? _advancedSearchCriteria;  // zuletzt verwendete erweiterte Suche
    ...
    private readonly List<FritzCallEvent> _recentCalls = [];
    ...
    // Entkopplung der Detailanzeige: Bei gedrückt gehaltener Pfeiltaste im DataGridView sollen
    // Foto, Gruppen und Dokumente nicht für jede übersprungene Zeile geladen werden.
    private DateTime _lastCurrentChanged = DateTime.MinValue;
    private bool _isCreatingImplicitRecord = false;  // verhindert Rekursion beim automatischen Anlegen eines Datensatzes
    private const int RapidNavigationMs = 250;  // Abstand, unterhalb dessen von "Durchblättern" ausgegangen wird

    #endregion

    #region Start, Initialisierung und Erscheinungsbild

    //[GeneratedRegex(@"(?:\+|†|verstorben|gestorben|verst\.|gest\.)(?:\s+am)?\s*(?<date>\d{1,2}\.\d{1,2}\.\d{2,4})", RegexOptions.IgnoreCase)]
    [GeneratedRegex(@"(?:\+|†|verstorben|gestorben|verst\.|gest\.)(?:\s+am)?\s*(?<date>\d{1,2}\.\s*(?:\d{1,2}\.|(?:Januar|Februar|März|April|Mai|Juni|Juli|August|September|Oktober|November|Dezember)\s+)\d{2,4})",
    ...
    public FrmAdressen(FrmSplashScreen? splashScreen, string[] args)
    {
        ...
        {
            if (arg.Equals("-min", StringComparison.OrdinalIgnoreCase))
            {
        ...
        typeof(Panel).InvokeMember("DoubleBuffered", BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.SetProperty, null, panelPictBox, [true]);  // die Foto-Toolbar wandert beim Ändern der Bildhöhe – ohne Doppelpufferung flackert das
        ...
        cbLand.AutoCompleteCustomSource.AddRange(CountryList.Names);  // Statische Länderliste als Vorschläge (AutoCompleteMode/Source sind bereits im Designer gesetzt)
        ...
        editControlsDictionary.Add(tbStrasse, "Strasse");
    ...
    private void DisplayPlaceholderText(bool show)
    {
        ...
        tbStrasse.PlaceholderText = show ? $"{AppSettings.TextBoxPaddingChar}Adresse" : "";
        ...
        searchTSTextBox.TextBox.PlaceholderText = show ? $"{AppSettings.TextBoxPaddingChar}Suche ( Strg+F)" : "";

        toolTip.SetToolTip(cbAnrede, show ? "" : "Anrede");
        toolTip.SetToolTip(cbPraefix, show ? "" : "Titel");
        toolTip.SetToolTip(tbVorname, show ? "" : "Vorname");
        toolTip.SetToolTip(tbZwischenname, show ? "" : "Zwischenname");
        toolTip.SetToolTip(tbNickname, show ? "" : "Nickname");
        toolTip.SetToolTip(tbNachname, show ? "" : "Nachname");
        toolTip.SetToolTip(tbSuffix, show ? "" : "Suffix");
        toolTip.SetToolTip(tbFirma, show ? "" : "Unternehmen");
        toolTip.SetToolTip(tbPosition, show ? "" : "Position");
        toolTip.SetToolTip(tbStrasse, show ? "" : "Adresse");
        toolTip.SetToolTip(cbPLZ, show ? "" : "PLZ");
        toolTip.SetToolTip(cbOrt, show ? "" : "Ort");
        toolTip.SetToolTip(tbPostfach, show ? "" : "Postfach");
        toolTip.SetToolTip(cbLand, show ? "" : "Land");
    ...
    private void ApplyMenuImages()
    {
        ...
        hideToolStripMenuItem.Image = Resources.HideMember16;
        ...
        serienbriefToolStripMenuItem.Image = Resources.document_stack16;
        ...
        pWAToolStripMenuItem.Image = Resources.mobilephone16;
        ...
        searchTSButton.Image = Resources.search_24;
    ...
    {
        saveTSButton.DisplayStyle = splitContainer.SplitterDistance > 500 ? ToolStripItemDisplayStyle.ImageAndText : ToolStripItemDisplayStyle.Image;
        ...
        var availableWidth = splitContainer.SplitterDistance - startX - clearLabelWidth - searchTSButton.Width - 1;
    ...
    {
        deleteTSButton.DisplayStyle =
        clipboardTSButton.DisplayStyle =
        copyTSButton.DisplayStyle =
            toolStrip.Width - splitContainer.SplitterDistance > 630 ? ToolStripItemDisplayStyle.ImageAndText : ToolStripItemDisplayStyle.Image;
    }
    ...
    #region Datenbank: Verbindung, Speichern, Import/Export
    ...
    private async Task ConnectSQLDatabaseAsync(string file)
    {
        ...
        try
        {
            ...
            if (addressDGV.Columns["Reminder"] is DataGridViewColumn col) { col.Visible = false; }  // Browsable = false bricht DataBinding, daher hier manuell verstecken
            PopulateMemberships();
            ...
            if (_context != null)
            {
                ...
                if (addressBSource.Count > 0) { LoadCurrentAddressDetails(); }  // direkt laden (ohne Verzögerung)

                if (!isMigrated && _settings.BirthdayAddressShow && !_startMinToTray)
                {
                ...
                LoadCurrentAddressDetails();  // Einmalig für den ersten Datensatz, direkt (ohne Verzögerung)
    ...
    private async Task<DialogResult> SaveSQLDatabaseAsync(bool closeDB = false, bool askNever = false, bool isFormClosing = false)
    {
        ...
        if (!askNever && _settings.AskBeforeSaveSQL)
        {
            ...
            var saveButton = new TaskDialogButton("&Speichern");  //TaskDialogButton.Yes; 
            var dontSaveButton = new TaskDialogButton("&Verwerfen"); //TaskDialogButton.No;
            ...
            if (result != cancelButton)  // CheckBox-Status nur übernehmen, wenn der Vorgang nicht komplett abgebrochen wurde
            {
                ...
                {
                    if (_settings.AskBeforeSaveSQL && !finalCheck.Checked) { _settings.AskBeforeSaveSQL = false; }
    ...
    #region Adressen-Tab: DataGridView und Statuszeile
    ...
    private void AddressBindingSource_CurrentChanged(object? sender, EventArgs e)
    {
        ...
        ScheduleDetailLoad(LoadCurrentAddressDetails);
    }

    // Das Laden der Details (Foto, Gruppen, Dokumente, Tooltips) ist teuer. Bei einzelnen Wechseln
    // (Mausklick, einzelner Tastendruck) sofort laden, bei schneller Folge (gedrückte Pfeiltaste)
    // erst, wenn der Nutzer stehen bleibt. Sonst staut sich die Nachrichtenschlange und die
    // Anzeige bleibt stehen, um beim Loslassen weit zu springen.
    private void ScheduleDetailLoad(Action loadNow)
    {
        var now = DateTime.UtcNow;
        var isRapid = (now - _lastCurrentChanged).TotalMilliseconds < RapidNavigationMs;
        _lastCurrentChanged = now;
        detailTimer.Stop();
        if (isRapid) { detailTimer.Start(); }   // Timer wird bei jedem weiteren Wechsel neu gestartet
        else { loadNow(); }
    }
    ...
    private void DetailTimer_Tick(object? sender, EventArgs e)
    {
        detailTimer.Stop();
        if (_isFiltering) { return; }
        if (tabControl.SelectedTab == addressTabPage) { LoadCurrentAddressDetails(); }
        else if (tabControl.SelectedTab == contactTabPage) { LoadCurrentContactDetails(); }
    }

    private void LoadCurrentAddressDetails()  // Detailanzeige (Foto, Gruppen, Dokumente, Alter) für den aktuellen Datensatz aufbauen
    {
        if (_isFiltering) { return; }
        try
        {
    ...
    #region Tab-Wechsel Adressen / Google-Kontakte
    ...
    private void SwitchDataBinding(BindingSource targetSource)
    {
        ...
        // Rückschreiben ins Datenobjekt: Bei den lokalen Adressen NIE automatisch, sondern nur über die
        // expliziten WriteValue()-Aufrufe in TextBox_TextChanged / TextBox_Leave / MaskedTextBox_*.
        // Grund: Mit OnPropertyChanged schreibt WinForms bei JEDER Änderung der Control-Eigenschaft zurück –
        // auch wenn AutoComplete oder die MaskedTextBox das TextChanged-Ereignis verzögert auslösen. Der
        // Rekursionsschutz von Binding.Target_PropertyChanged greift nur bei synchronen Ereignissen, sodass
        // beim schnellen Blättern der Text des vorigen Datensatzes in den bereits aktuellen geschrieben wird.
        ...
        foreach (var (control, dataMember) in editControlsDictionary)
        {
            ...
            var textBinding = new Binding("Text", targetSource, dataMember, true, updateMode) { NullValue = string.Empty };
        ...
        var birthdayBinding = new Binding("Text", targetSource, "Geburtstag", true, updateMode);  // s. o.: lokal nur per WriteValue()
        ...
        {
            ckbReminding.DataBindings.Add(new Binding("Checked", targetSource, nameof(Adresse.Reminder), true, DataSourceUpdateMode.Never));  // nur per WriteValue() in CkbReminding_CheckedChanged
        }
    }
    ...
    private void TabControl_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            hideToolStripMenuItem.Text = "Kontakt ausblenden";
            ...
            if (contactBSource.DataSource != null)
            {
                ...
                else { topAlignZoomPictureBox.Image = Resources.Avatar150; }
            }
            // UI Status
            if (contactBSource.Count > 0)
            {
                ...
                newToolStripMenuItem.Enabled = duplicateToolStripMenuItem.Enabled = deleteToolStripMenuItem.Enabled = hideToolStripMenuItem.Enabled = false;
    ...
    private void SetCommonButtonState(bool enabled)
    {
        ...
        deleteToolStripMenuItem.Enabled = deleteTSButton.Enabled = hideToolStripMenuItem.Enabled =
    ...
    #region Detailansicht: Eingabefelder, Datum, AutoVervollständigung
    ...
    private void AgeLabel_MaskedTB_Set(DateOnly birthDate)
    {
        ...
        if (TryGetDeathDate(out var deathDate) && deathDate > birthDate)
        {
            ...
            ageLabel.Text = $"im Alter von {ageText} verstorben";
        ...
        if (Math.Abs(days) <= 31)
        {
            ageLabel.Text = Math.Abs(days) == 1 ? days.ToString() + " Tag" : days.ToString() + " Tage";
        }
        else
        {
            var ddf = Utils.CalcDateDiff(todayAsDateTime, dateAsDateTime);
            ageLabel.Text =
                (ddf.Years != 0 ? ddf.Years + (ddf.Years == 1 ? " Jahr" : " Jahre") +
                    (ddf.Months == 0 && ddf.Days == 0 ? "" : ", ") : "") +
                (ddf.Months != 0 ? ddf.Months + (ddf.Months == 1 ? " Monat" : " Monate") +
                    (ddf.Days == 0 ? "" : ", ") : "") +
                (ddf.Days != 0 ? ddf.Days + (ddf.Days == 1 ? " Tag" : " Tage") : "");
    ...
    {
        deathDate = default;
        var match = DeathDateRegex().Match(tbBetreff.Text);
        if (match.Success && DateOnly.TryParseExact(match.Groups["date"].Value, formats, culture, DateTimeStyles.None, out deathDate)) { return true; }
        return false;
    }
    ...
    private void TextBox_KeyDown(object sender, KeyEventArgs e)
    {
        if (e.KeyCode == Keys.Enter)
        {
            ...
            if (!NativeMethods.IsPhysicalEnterPressed())  // Dropdown-Fall wird in ProcessCmdKey abgefangen; hier nur noch synthetisches Enter nach Mausklick
            {
                if (sender is TextBox tbb) { tbb.SelectionStart = tbb.Text.Length; tbb.SelectionLength = 0; }
            }
            else
            {
                if (sender is TextBox tbb) { tbb.SelectionStart = tbb.Text.Length; tbb.SelectionLength = 0; }  // Selektion nach AutoComplete aufheben
    ...
    private async void TextBox_Enter(object sender, EventArgs e)
    {
        if (sender is TextBox tb)
        {
            ...
            var isGeoControl = tb == cbPLZ || tb == cbOrt || tb == tbStrasse;
            // IAutoComplete::Init() sendet WM_SETFOCUS → zweites Enter während des Geo-Awaits.
            // In diesem Re-Entry _lastFocusWasMouse NICHT überschreiben.
            var isIAutoCompleteReEntry = isGeoControl && _geoLoading && tb == _lastFocusedGeoControl;

            if (!isIAutoCompleteReEntry) { _lastFocusWasMouse = MouseButtons != MouseButtons.None; }

            if (isGeoControl && !_geoLoading && _lastFocusedGeoControl != tb)
            {
            ...
            // Tastaturnavigation (Tab/Enter) → SelectAll; Mausklick → Cursor bleibt an Klickposition
            if (!_lastFocusWasMouse) { tb.SelectAll(); }
        }
    }
    ...
    private void LoadFallbackSuggestions(TextBox targetTextBox)
    {
        ...
        if (tabControl.SelectedTab == addressTabPage && _context != null)  // die richtige In-Memory-Datenquelle durchsuchen
        {
            foreach (var item in _context.Adressen.Local)
            {
                ...
                else if (targetTextBox == tbStrasse && !string.IsNullOrWhiteSpace(item.Strasse)) { suggestions.Add(item.Strasse); }
            }
        }
    ...
    {   // Console.Beep(); // beept ohne _lastFocusedGeoControl 3x und ohne _geoLoading 2x
        if (GeoDbContext.DatabaseExists)
        {
            try
            {
                ...
                else if (targetTextBox == tbStrasse)
                {
    ...
    private void TextBox_Leave(object sender, EventArgs e)
    {
        ...
        {
            var eingabe = tb.Text;
            if (!string.IsNullOrEmpty(eingabe))  // nachfolgender Code sorgt für korrekte Großschreibung (getippt "musterstraße 14a" → "Musterstraße 14a")
            {
            ...
            // Wert jetzt in die Datenbank schreiben
            var isLocal = tabControl.SelectedTab == addressTabPage;
            if (isLocal && (tb == tbMail1 || tb == tbMail2))
            {
                tb.DataBindings["Text"]?.WriteValue();
                UpdateSaveButton();  // Mail-Felder werden erst hier geschrieben → Speichern-Status jetzt neu bewerten
            }

            // Mail-Validierung: unvollständige Adresse → roter Hintergrund
            var normalBack = _isDarkMode ? Color.FromArgb(45, 45, 45) : Color.White;
            if ((tb == tbMail1 || tb == tbMail2) && !string.IsNullOrEmpty(tb.Text.Trim()))
            {
                var at = tb.Text.IndexOf('@');
                var valid = at > 0 && tb.Text.LastIndexOf('.') > at + 1 && tb.Text.Length > at + 3;
                tb.BackColor = valid ? normalBack : (_isDarkMode ? Color.FromArgb(100, 30, 30) : Color.MistyRose);
            }
            else
            {
            ...
            tb.ForeColor = _isDarkMode ? Color.White : Color.Black;
            LinkLabel_Enabled();
            if (tb == cbPLZ || tb == cbOrt || tb == tbStrasse) { _lastFocusedGeoControl = null; }
        }
    }
    ...
    private void MaskedTextBox_Enter(object sender, EventArgs e)
    {
        ...
        _maskedTbFocusClick = true; // nächster MouseDown soll Sektion selektieren
    ...
    private void MaskedTextBox_MouseDown(object sender, MouseEventArgs e)
    {
        ...
        {
            if (!_maskedTbFocusClick) { return; } // Feld bereits fokussiert → kein Auto-Selekt, Cursor bleibt wo er ist
            _maskedTbFocusClick = false;
    ...
    private void BtnResetDate_Click(object sender, EventArgs e)
    {
        ...
        {
            var (isYes, _) = Utils.YesNo_TaskDialog(this, "Adressen", "Möchtest du das Datum entfernen?", adresse.Geburtstag.ToString() ?? "", "&Löschen", "&Behalten");
            if (!isYes) { return; }
        ...
        {
            var (isYes, _) = Utils.YesNo_TaskDialog(this, "Kontakte", "Möchtest du das Datum entfernen?", contact.Geburtstag.ToString() ?? "", "&Löschen", "&Behalten");
            if (!isYes) { return; }
    ...
    private void TextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        // Sonderfall "kein Datensatz angezeigt" (z. B. Suche ohne Treffer, aktiver Filter oder leere Liste):
        // Die Editierfelder sind dann leer, aber bedienbar. Tippt der Anwender hier los, will er
        // offensichtlich einen neuen Datensatz anlegen – also legen wir ihn automatisch an.
        var noRecordShown = isLocal
            ? _context != null && addressBSource?.DataSource != null && addressBSource.Current is not Adresse
            : contactBSource?.DataSource != null && contactBSource.Current is not Contact;
        if (noRecordShown && !_isCreatingImplicitRecord)
        {
            CreateRecordFromTyping(senderControl);
            return;
        }

        var isMailControl = senderControl == tbMail1 || senderControl == tbMail2;

        // Das sofortige WriteValue() für Mail-Felder aussetzen, um das AutoComplete nicht zu stören!
        if (isLocal && !isMailControl) { senderControl.DataBindings["Text"]?.WriteValue(); }
        ...
        if (((senderControl == tbBetreff) && DeathDateRegex().IsMatch(senderControl.Text)) || ageLabel.Text.EndsWith("verstorben"))
        {
            ...
            if (isLocal && addressBSource is not null && addressBSource.Current is Adresse addr) { geburtsdatum = addr.Geburtstag; }
            else if (isGoogle && contactBSource is not null && contactBSource.Current is Contact cont) { geburtsdatum = cont.Geburtstag; }
            if (geburtsdatum.HasValue) { AgeLabel_MaskedTB_Set(geburtsdatum.Value); }
        }

        if (isMailControl && sender is TextBox tbMail)
        {
            var text = tbMail.Text;
            if (string.IsNullOrEmpty(text)) { return; }

            var atIdx = text.IndexOf('@');
            // Fall 1: Kein @-Zeichen (mehr) vorhanden
            if (atIdx < 0)
            {
                if (tbMail.AutoCompleteCustomSource.Count > 0)
                {
                    BeginInvoke(() =>
                    {
                        if (tbMail.IsDisposed) { return; }
                        var pos = tbMail.SelectionStart;
                        tbMail.AutoCompleteMode = AutoCompleteMode.None;
                        tbMail.AutoCompleteCustomSource.Clear();
                        tbMail.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
                        tbMail.SelectionStart = pos;
                        tbMail.SelectionLength = 0;
                    });
                }
                return;
            }
            // Fall 2: @-Zeichen vorhanden -> Liste aufbauen (verhindert das Verschlucken des ersten Buchstabens)
            var prefix = text[..(atIdx + 1)];
            if (tbMail.AutoCompleteCustomSource.Count > 0 && tbMail.AutoCompleteCustomSource[0].StartsWith(prefix, StringComparison.OrdinalIgnoreCase)) { return; }
            var suggestions = _eMailDomains.Select(d => prefix + d).ToArray();
            BeginInvoke(() =>
            {
                if (tbMail.IsDisposed) { return; }
                var pos = tbMail.SelectionStart;
                tbMail.AutoCompleteMode = AutoCompleteMode.None;
                tbMail.AutoCompleteCustomSource.Clear();
                tbMail.AutoCompleteCustomSource.AddRange(suggestions);
                tbMail.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
                tbMail.SelectionStart = pos;
                tbMail.SelectionLength = 0;
            });
    ...
    private void BuildEmailDomains()
    {
        var isContactTab = tabControl.SelectedTab == contactTabPage;
        var mails = isContactTab
            ? contactBSource.OfType<Contact>().SelectMany(c => (string?[])[c.Mail1, c.Mail2])
            : addressBSource.OfType<Adresse>().SelectMany(a => (string?[])[a.Mail1, a.Mail2]);
        var fromData = mails
            .Where(m => !string.IsNullOrWhiteSpace(m) && m!.Contains('@'))
            .Select(m => { var i = m!.IndexOf('@'); return i >= 0 ? m[(i + 1)..].ToLowerInvariant().Trim() : null; })
            .OfType<string>()
            .Where(d => d.Contains('.') && d.Length > 3)
            .GroupBy(d => d, StringComparer.OrdinalIgnoreCase)
            .OrderByDescending(g => g.Count())
            .Select(g => g.Key);
        _eMailDomains = [.. fromData.Concat(_eMailDomainsStandard).Distinct(StringComparer.OrdinalIgnoreCase)];
        var mailControls = new TextBox[] { tbMail1, tbMail2 };
        foreach (var tb in mailControls)
        {
            if (tb != null)  // Hier prüfen wir sicherheitshalber auf null, falls das Control in einem Designer-State noch nicht vollständig initialisiert wäre
            {
                if (tb.AutoCompleteCustomSource.Count > 0)
                {
    ...
    private void UpdatePlaceholderVis()  // erzeugt ein Label in der FlowLayoutPanel, wenn keine Tags vorhanden sind, und entfernt es, sobald Tags existieren
    {
    ...
    private void AdaptReminderLayout()
    {
        ...
        {
            // Rein optisches Zurücksetzen bei Adressen ohne Geburtstag. Der Wert darf NICHT nach
            // Adresse.Reminder durchgeschrieben werden – deshalb ist das Binding mit
            // DataSourceUpdateMode.Never angelegt (s. SwitchDataBinding).
            ckbReminding.Checked = false;
            ckbReminding.Text = string.Empty;
            return;
        }
        ckbReminding.Text = panelBirthdayClearRemind.Width > 200 ? "Erinnerung" : "remind";
    }

    #endregion

    #region Datensätze: Neu, Duplizieren, Löschen, Speichern
    ...
    // Wird aus TextBox_TextChanged aufgerufen, wenn der Anwender in ein Editierfeld tippt, obwohl gar kein
    // Datensatz angezeigt wird (Suche ohne Treffer, aktiver Filter oder leere Liste). Ohne diese Behandlung
    // liefen die Eingaben ins Leere, weil die Datenbindung kein Ziel hat. Gilt für beide Tabs.
    private async void CreateRecordFromTyping(Control senderControl)
    {
        if (_isCreatingImplicitRecord) { return; }
        var isAddressTab = tabControl.SelectedTab == addressTabPage;
        _isCreatingImplicitRecord = true;
        try
        {
            // Eingabe sichern: Beim Zurücksetzen von Suche/Filter und beim Anlegen des neuen
            // Datensatzes überschreibt die Datenbindung den Feldinhalt.
            var typedText = senderControl.Text;
            var caret = senderControl switch
            {
                TextBoxBase tb => tb.SelectionStart,
                ComboBox cb => cb.SelectionStart,
                _ => typedText.Length
            };

            // Zwingend nötig: Solange gefiltert wird, hängt die BindingSource an einer temporären
            // BindingList. Ein dort hinzugefügter Datensatz erreichte weder den EF-ChangeTracker
            // noch _allGoogleContacts und wäre beim Speichern verloren.
            if (!string.IsNullOrEmpty(searchTSTextBox.Text) || filterRemoveToolStripMenuItem.Visible)
            {
                await Clear_Search_Remove_Filter();
            }

            isSelectionChanging = true;  // Lock für RowValidating (wie bei "hinzufügen")
            try
            {
                _lastProcessedEntry = null;
                if (isAddressTab)
                {
                    var newAddress = addressBSource.AddNew();
                    addressBSource.EndEdit();  // setzt den Status im EF ChangeTracker zuverlässig auf 'Added'
                    SelectNewRow(addressBSource, addressDGV, newAddress);
                }
                else
                {
                    var newContact = new Contact();
                    contactBSource.Add(newContact);
                    contactBSource.ResetBindings(false);
                    SelectNewRow(contactBSource, contactDGV, newContact);

                    // Interne Referenzen auf den NEUEN Kontakt biegen. Wichtig: Der Schnappschuss
                    // entsteht hier noch vom LEEREN Kontakt, damit die Eingabe als Änderung zählt.
                    _lastActiveContact = newContact;
                    _originalContactSnapshot = (Contact)newContact.Clone();
                    ignoreTextChange = true;
                    try
            ...
            ignoreTextChange = true;  // Eingabe zurückschreiben, ohne diese Methode erneut auszulösen
            try { senderControl.Text = typedText; }
            finally { ignoreTextChange = false; }
            senderControl.DataBindings["Text"]?.WriteValue();  // lokale Bindings stehen auf Never → explizit schreiben

            if (senderControl.CanFocus) { senderControl.Focus(); }  // Clear_Search_Remove_Filter setzt den Fokus ins Suchfeld
            switch (senderControl)  // Schreibmarke wieder an die getippte Stelle
            {
                case TextBoxBase tb:
                    ...
                    break;
                case ComboBox cb:
                    cb.SelectionStart = Math.Min(caret, cb.Text.Length);
        ...
        finally { _isCreatingImplicitRecord = false; }
    }

    private static void SelectNewRow(BindingSource bs, DataGridView dgv, object? newItem)
    {
        if (newItem == null) { return; }
        var realIndex = bs.IndexOf(newItem);
        if (realIndex < 0) { return; }
        bs.Position = realIndex;
        if (dgv.RowCount > realIndex)
        {
            dgv.FirstDisplayedScrollingRowIndex = realIndex;
            dgv.Rows[realIndex].Selected = true;
        }
    }
    ...
    private async void SearchTSButton_Click(object sender, EventArgs e)
    {
        if (!await ContactChanges_Check()) { return; }
        var isFilterActive = filterRemoveToolStripMenuItem.Visible;
        using var dlg = new FrmAdvSearch(_settings.ColorScheme, isFilterActive);
        if (_advancedSearchCriteria != null) { dlg.ApplyCriteria(_advancedSearchCriteria); }

        if (dlg.ShowDialog(this) != DialogResult.OK) { return; }

        var criteria = dlg.BuildCriteria();
        if (criteria.IsEmpty)
        {
            if (!dlg.RefineSearch) { FilterRemoveToolStripMenuItem_Click(sender, e); }
            return;
        }

        _advancedSearchCriteria = criteria;
        ApplyAdvancedSearch(criteria, dlg.RefineSearch);
    }

    private void ApplyAdvancedSearch(AdvancedSearchCriteria criteria, bool refineSearch = false)
    {
        var isAddressTab = tabControl.SelectedTab == addressTabPage;
        ...
        if (activeBs.Current != null) { activeBs.EndEdit(); }
        ...
        try
        {
            ...
            {
                var source = refineSearch ? activeBs.Cast<Adresse>() : _context.Adressen.Local.AsEnumerable();
                var predicate = criteria.BuildAdressePredicate();
                filtered = source.Where(predicate).ToList();
                if (activeBs.DataSource is not System.Collections.IList cur || !cur.Cast<Adresse>().SequenceEqual(filtered.Cast<Adresse>()))
                {
            ...
            {
                var source = refineSearch ? activeBs.Cast<Contact>() : _allGoogleContacts.AsEnumerable();
                var predicate = criteria.BuildContactPredicate();
                filtered = source.Where(predicate).ToList();
                if (activeBs.DataSource is not System.Collections.IList cur || !cur.Cast<Contact>().SequenceEqual(filtered.Cast<Contact>()))
                {
                    activeBs.DataSource = filtered;
                }
                UpdateContactStatusBar();
        ...
        finally
        {
            ...
            {
                _lastProcessedEntry = null;  // Foto/Details-Update erzwingen, auch wenn Position 0 schon aktiv war
                if (activeBs.Count > 0)
                {
                    SyncGridToPosition(activeDgv, activeBs, 0, setFocus: true);
                    activeBs.ResetCurrentItem();
                }
                if (isAddressTab) { LoadCurrentAddressDetails(); }  // direkt laden (ohne Verzögerung)
                else { LoadCurrentContactDetails(); }  // direkt laden (ohne Verzögerung)
            });
        }
    }
    ...
    private void ApplyGlobalSearch(string searchText, bool jumpToFirstRow = true)
    {
        ...
        try
        {
            ...
            if (isAddressTab && _context != null)
            {
                ...
                System.Collections.IList filtered;
                if (isSearchEmpty) { filtered = source.ToBindingList(); }
                else { filtered = source.Where(a => terms.All(t => a.SearchText.Contains(t))).ToList(); }

                // 2. Flackerschutz: Nur bei echten Änderungen die DataSource tauschen
                if (activeBs.DataSource is not System.Collections.IList currentList || !currentList.Cast<Adresse>().SequenceEqual(filtered.Cast<Adresse>())) { activeBs.DataSource = filtered; }
                UpdateAddressStatusBar();
            }
            else if (!isAddressTab && _allGoogleContacts != null)
            {
                System.Collections.IList filtered;
                if (isSearchEmpty) { filtered = _allGoogleContacts; }
                else { filtered = _allGoogleContacts.Where(c => terms.All(t => c.SearchText.Contains(t))).ToList(); }
                if (activeBs.DataSource is not System.Collections.IList currentList || !currentList.Cast<Contact>().SequenceEqual(filtered.Cast<Contact>())) { activeBs.DataSource = filtered; }
                UpdateContactStatusBar();
            }
        }
        catch (Exception ex)
            ...
            if (activeBs.Count > 0)
            {
                ...
                _ = activeDgv.InvokeAsync(() =>
                {
                    ...
                    if (_lastProcessedEntry != currentEntry)
                    {
                        ...
                        if (isAddressTab)
                        {
                            LoadCurrentAddressDetails();  // direkt laden (ohne Verzögerung)
                        }
                        else
                        {
                            LoadCurrentContactDetails();  // direkt laden (ohne Verzögerung)
                        }
                    }
                });
            }
    ...
    private void SearchTSTextBox_KeyDown(object sender, KeyEventArgs e)
    {
        ...
        else if (e.KeyCode == Keys.Enter || e.KeyCode == Keys.Return || e.KeyCode == Keys.Down)
        {
            ...
            {
                var row = contactDGV.Rows.Cast<DataGridViewRow>().FirstOrDefault(row => row.Visible);
                if (row != null)
                {
                    contactDGV.Focus();
                    row.Selected = true;
                    contactDGV.FirstDisplayedScrollingRowIndex = row.Index;
                }
            }
    ...
    private void TsClearLabel_VisibleChanged(object sender, EventArgs e) => UpdateSearchBoxWidth();
    ...
    {
        _advancedSearchCriteria = null;  // erweiterte Suche zurücksetzen
    ...
    private void HideToolStripMenuItem_Click(object sender, EventArgs e)
    {
        if (tabControl.SelectedTab == addressTabPage && _context != null)
        {
            ...
            HideCurrentItem(_context.Adressen.Local, addressBSource, addressDGV, current, "Adressen");
        }
        else if (tabControl.SelectedTab == contactTabPage && _allGoogleContacts != null)
        {
            if (contactBSource.Current is not Contact current) { return; }
            HideCurrentItem(_allGoogleContacts, contactBSource, contactDGV, current, "Google Kontakte");
        }
    }

    private void HideCurrentItem<T>(IEnumerable<T> sourceList, BindingSource bs, DataGridView dgv, T currentItem, string entityName) where T : class
    {
        var oldPosition = bs.Position;  // Nachfolger des ausgeblendeten Eintrags soll an dessen Position rücken
        var remaining = bs.List.Cast<T>().Where(item => !ReferenceEquals(item, currentItem)).ToHashSet();  // Basis ist die aktuelle BindingList
        ExecuteFilter(sourceList, bs, dgv, remaining.Contains, "ausgeblendet", entityName, oldPosition);
    }
    ...
    private void ExecuteFilter<T>(IEnumerable<T> sourceList, BindingSource bs, DataGridView dgv, Func<T, bool> predicate, string statusText, string entityName, int selectIndex = 0)
    {
        ...
        var firstDisplayedRow = dgv.FirstDisplayedScrollingRowIndex;  // Scrollposition merken (für Wiederherstellung bei selectIndex > 0)
        try
        {
            dgv.SuspendLayout();
            currencyManager?.SuspendBinding();
        ...
        finally
        {
            ...
            dgv.ResumeLayout();
            // Zielzeile erst NACH ResumeBinding setzen: Vorher würde der CurrencyManager das Grid wieder auf Position 0 zurücksynchronisieren.
            // Da alles im selben UI-Durchlauf passiert, wird nur der Endzustand gezeichnet (kein sichtbarer Zwischensprung über Zeile 0).
            if (selectIndex > 0 && bs.Count > 0 && dgv.Rows.Count > 0)
            {
                // 1. Scrollposition wiederherstellen, damit das Setzen der CurrentCell in SyncGridToPosition nicht scrollt (Zeile ist dann bereits sichtbar)
                if (firstDisplayedRow > 0) { dgv.FirstDisplayedScrollingRowIndex = Math.Min(firstDisplayedRow, dgv.Rows.Count - 1); }
                // 2. Zielzeile selektieren; SyncGridToPosition scrollt nur (zentriert), falls die Zeile nicht vollständig im Sichtfeld liegt
                SyncGridToPosition(dgv, bs, Math.Min(selectIndex, bs.Count - 1), !searchTSTextBox.Focused);
            }
            _isFiltering = false;
            if (typeof(T) == typeof(Adresse)) { LoadCurrentAddressDetails(); }  // Detailanzeige direkt aufbauen (ohne Verzögerung)
            else if (typeof(T) == typeof(Contact)) { LoadCurrentContactDetails(); }  // Detailanzeige direkt aufbauen (ohne Verzögerung)
            UpdateFilterUIState();
        }
    }
    ...
    private void GroupFilterToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        // 2. Dialog anzeigen
        using var frm = new FrmGroupFilter(dialogGroups, isFilterActive, _settings.ColorScheme);
    ...
    private void GlobalSearchToolStripMenuItem_Click(object sender, EventArgs e) => SearchTSButton_Click(sender, EventArgs.Empty);

    #endregion

    #region Google-Kontakte: Laden, Speichern, Abgleich
    ...
    private async Task LoadAndDisplayGoogleContactsAsync()
    {
        ...
        try
        {
            ...
            // Geburtstagserinnerung nur unterdrücken, wenn wirklich eine Browser-Anmeldung stattfand.
            if (manager.LastAuthorizationWasInteractive) { contactBirthdayFlag = false; }
            ...
            try
            {
                ...
                BuildEmailDomains(); // nach Tab-Wechsel: contactTabPage ist jetzt SelectedTab
                Text = $"Kontakte - Google Kontakte";
            }
            finally { isSelectionChanging = false; }
            ...
            if (hasRows)
            {
                ...
                LoadCurrentContactDetails();  // dadurch wird _lastActiveContact aktualisiert, Foto geladen, etc. (direkt, ohne Verzögerung)
                contactBSource.ResetBindings(false);  // Der WinForms-Hammer: Zwingt alle Editier-Textboxen rechts zum sofortigen Refresh!
            }
            else { LoadCurrentContactDetails(); }  // Auch bei einer leeren Liste müssen wir aufräumen (Felder leeren)
    ...
    private void ContactBindingSource_CurrentChanged(object? sender, EventArgs e)
    {
        ...
        // Leichtgewichtig und deshalb sofort: Snapshot für die Änderungserkennung mitführen.
        // Nur die teure Detailanzeige (Foto, Tags, Tooltips) wird beim Durchblättern verzögert.
        if (contactBSource.Current is Contact current)
        {
            _lastActiveContact = current;
            _originalContactSnapshot = (Contact)current.Clone();
        ...
        ScheduleDetailLoad(LoadCurrentContactDetails);
    }

    private void LoadCurrentContactDetails()  // Detailanzeige für den aktuellen Google-Kontakt aufbauen
    {
        if (_isFiltering || isSelectionChanging) { return; }
        ...
        try
        {
            if (!ReferenceEquals(_lastActiveContact, contact))  // beim Aufruf über CurrentChanged bereits gesetzt; hier nur für die direkten Aufrufe
            {
    ...
    private void AuthentMenuItem_Click(object sender, EventArgs e)
    {
        ...
        {
            var tokenFile = Path.Combine(tokenDir, GooglePeopleManager.TokenFileName);
    ...
    private void ProcessGoogleContactPhoto(Image rawImage, ImageFormat rawFormat, Contact targetContact)
    {
        ...
        try
        {
            // 1. Skalierung: Fotos für Google-Kontakte einheitlich auf AppSettings.GooglePhotoWidth bringen.
            // Schwelle und Zielbreite sind bewusst derselbe Wert – sonst würde ein Bild knapp über der
            // Schwelle stärker verkleinert als eines knapp darunter.
            if (rawImage.Width > AppSettings.GooglePhotoWidth)
            {
                workingImage = Utils.SkaliereBildDaten(rawImage, AppSettings.GooglePhotoWidth);
                isScaled = true;
            }
            ...
            TaskDialogRadioButton? centerRadio = null, topRadio = null, bottomRadio = null, skipRadio = null;
            ...
            if (workingImage.Height > workingImage.Width && workingImage.Width > topAlignZoomPictureBox.Width)
            {
                ...
                bottomRadio = new TaskDialogRadioButton("&Unten priorisieren, nur oben abschneiden");
                skipRadio = new TaskDialogRadioButton("&Nicht beschneiden (nicht empfohlen)");
                radioButtons.AddRange([topRadio, centerRadio, bottomRadio, skipRadio]);
            ...
            // Auch die Höhe wird begrenzt (weiter unten, nach dem Zuschnitt) – der Hinweis gilt daher für beide Richtungen.
            var isLimited = isScaled || workingImage.Height > AppSettings.GooglePhotoWidth;
            var scaledHint = isLimited ? $"\nDas Bild wird auf {AppSettings.GooglePhotoWidth} px Kantenlänge verkleinert." : string.Empty;
            ...
            initialPage.Buttons[1].Click += (sender, ev) =>
            {
                ...
                else if (bottomRadio?.Checked == true)
                {
                    intermediateImageToDispose = workingImage;
                ...
                finalImageForDisplay = skipRadio?.Checked == true ? Utils.ReduziereWieGoogle(workingImage, AppSettings.PictBoxWidth) : (Image)workingImage.Clone();
            ...
            // 3. Höhe begrenzen – bewusst ERST HIER, nach dem Zuschnitt: Beim Beschneiden zum Quadrat
            // entspricht die Kantenlänge der Bildbreite. Eine Höhenbegrenzung vorab würde ein Hochformat
            // erst schmal rechnen und damit unnötig Auflösung für das Quadrat verschenken.
            // Betroffen bleiben also nur Hochformate ohne Zuschnitt (abgelehnt oder zu schmal für die Abfrage).
            if (workingImage!.Height > AppSettings.GooglePhotoWidth)
            {
                var reducedImage = Utils.ReduziereWieGoogle(workingImage, AppSettings.GooglePhotoWidth);
                workingImage.Dispose();
                workingImage = reducedImage;
            }

            // 4. Foto vormerken und anzeigen
    ...
    private async void PasteToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        if (tabControl.SelectedTab == addressTabPage && addressBSource.Current is Adresse adresse)
        {
            try
            {
                ...
                Image finalImage;
                // Lokale Adressen: Wir skalieren auf 150 Pixel Breite
                if (clipboardImage.Width > AppSettings.PictBoxWidth)
                {
                    finalImage = Utils.SkaliereBildDaten(clipboardImage, AppSettings.PictBoxWidth);
        ...
        if (tabControl.SelectedTab == addressTabPage)
        {
            if (addressBSource.Current is Adresse adresse)
            {
                ...
                try
                {
                    ...
                    if (loadedImage.Width > AppSettings.PictBoxWidth)
                    {
                        scaledImage = Utils.SkaliereBildDaten(loadedImage, AppSettings.PictBoxWidth);
    ...
    private async void DelPictboxToolStripButton_Click(object sender, EventArgs e)
    {
        ...
        else if (tabControl.SelectedTab == contactTabPage && contactBSource.Current is Contact googleKontakt)
        {
            ...
            if (string.IsNullOrEmpty(googleKontakt.ResourceName))
            {
                ...
                topAlignZoomPictureBox.Image = Resources.Avatar150; // Standard-Icon zurücksetzen
                delPictboxToolStripButton.Enabled = false;
                UpdateSaveButton();
                return;
            }
            ...
            try
            {
                ...
                topAlignZoomPictureBox.Image = Resources.Avatar150; // Spezielles Kontakt-Icon
                delPictboxToolStripButton.Enabled = false;
    ...
    #region Dokumente: Doku-Tab, Verknüpfungen, Dateiüberwachung
    ...
    private void DokuPlusTSButton_Click(object sender, EventArgs e)
    {
        ...
        {
            var processedPaths = openFileDialog.FileNames.ToList();
            var single = processedPaths.Count == 1;
            var sourceDir = Path.GetDirectoryName(openFileDialog.FileName);
            if (!string.IsNullOrEmpty(sourceDir) && !string.IsNullOrEmpty(_settings.DocumentFolder) && !string.Equals(sourceDir, _settings.DocumentFolder, StringComparison.OrdinalIgnoreCase))
            {
                var btnCopy = new TaskDialogCommandLinkButton($"Datei{(single ? "" : "en")} kopieren", "und Kopie verknüpfen");
                var btnMove = new TaskDialogCommandLinkButton($"Datei{(single ? "" : "en")} verschieben", "und Verknüpfung erstellen");
                var page = new TaskDialogPage
                {
                    Caption = $"Datei{(single ? "" : "en")} außerhalb des Dokumentenordners",
                    Heading = $"Datei{(single ? "" : "en")} befinde{(single ? "t" : "n")} sich nicht in »{_settings.DocumentFolder}«",
                    Text = $"{string.Join(Environment.NewLine, processedPaths.Select(p => p.ToString()))}",
                    Icon = TaskDialogIcon.Warning,
                    Buttons = { btnCopy, btnMove, TaskDialogButton.Cancel },
                    Footnote = "Wähle »Abbrechen«, um den ursprünglichen Ort zu behalten.\nDer Dokumentenordner wird in den Einstellungen festgelegt.",
                    AllowCancel = true,
                    SizeToContent = true
                };
                var clickedButton = TaskDialog.ShowDialog(this, page);
                if (clickedButton == btnCopy || clickedButton == btnMove)
                {
                    fileSysWatcher?.EnableRaisingEvents = false;  // FileSystemWatcher pausieren, um eigene Events zu unterdrücken
                    try
                    {
                        processedPaths.Clear();
                        foreach (var pfad in openFileDialog.FileNames)
                        {
                            var fileName = Path.GetFileName(pfad);
                            var targetPath = Path.Combine(_settings.DocumentFolder, fileName);
                            if (!string.Equals(pfad, targetPath, StringComparison.OrdinalIgnoreCase))
                            {
                                try
                                {
                                    if (clickedButton == btnCopy) { File.Copy(pfad, targetPath, overwrite: true); }
                                    else if (clickedButton == btnMove) { File.Move(pfad, targetPath, overwrite: true); }
                                    processedPaths.Add(targetPath);
                                }
                                catch (Exception ex)
                                {
                                    Utils.ErrTaskDlg(Handle, ex);
                                    processedPaths.Add(pfad);  // Im Fehlerfall den ursprünglichen Pfad übernehmen
                                }
                            }
                            else { processedPaths.Add(targetPath); }
                        }
                    }
                    finally { fileSysWatcher?.EnableRaisingEvents = true; }  // FileSystemWatcher verlässlich wieder aktivieren
                }
            }
            foreach (var pfad in processedPaths) { Add2dokuListView(new FileInfo(pfad), false); }
            dokuListView.ListViewItemSorter = new ListViewItemComparer();
            dokuListView.Sort();
            SyncDocumentsToEntity();
        }
    ...
    private void SyncDocumentsToEntity()
    {
        ...
        var itemsToDelete = selectedAddress.Dokumente.Where(doc => !currentUiPaths.Contains(doc.Dateipfad)).ToList();
        foreach (var doc in itemsToDelete) { selectedAddress.Dokumente.Remove(doc); }
    ...
    #region Word, LibreOffice und Serienbrief
    ...
    private void LibreProcess()
    {
        ...
        else if (NativeMethods.GetLastVisibleHandleByTitleEnd("– LibreOffice Writer") != IntPtr.Zero) // geöffnentes Writer-Dokument
        {
            Process.Start(new ProcessStartInfo
            {
                ...
                //Arguments = $"\"{JsonSerializer.Serialize(bookmarkTextDictionary)}\"",
    ...
    private async void WordProcess()
    {
        FillWordProcessingDictionary(); // bookmarkTextDictionary mit aktuellen Werten füllen
        if (WordManager.TransferDataToActiveDocument(bookmarkTextDictionary, Handle)) { await OpenSettingsAsync(); }
    }

    private void SerienbriefToolStripMenuItem_Click(object sender, EventArgs e)
    {
        var isAddressTab = tabControl.SelectedTab == addressTabPage;
        var activeBs = isAddressTab ? addressBSource : contactBSource;
        if (activeBs is null || activeBs.Count == 0)
        {
            Utils.MsgTaskDlg(Handle, "Keine Adressen", "Es sind keine Datensätze für den Serienbriefdruck vorhanden.");
            return;
        }

        var useWord = _settings.WordProcessorProgram ?? Utils.AskWordProcessingProgram(Handle);
        if (useWord is null) { return; } // Abbruch

        if (useWord == true && !WordManager.IsWordInstalled)
        {
            Utils.MsgTaskDlg(Handle, "Word fehlt", "Microsoft Word wurde nicht gefunden. Bitte installiere es.");
            return;
        }
        if (useWord == false && !WordManager.IsLibreOfficeInstalled)
        {
            Utils.MsgTaskDlg(Handle, "LibreOffice fehlt", "LibreOffice Writer wurde nicht gefunden. Bitte installiere es.");
            return;
        }
        var entities = activeBs.List.Cast<object>().ToList();  // (gefilterte) Datensätze einsammeln – funktioniert für Adresse und Contact
        string csvPath;
        int count;
        try
        {
            csvPath = Path.Combine(WordManager.GetDownloadsPath(), "Adressen", MailMergeManager.CsvFileName);
            count = MailMergeManager.WriteCsv(csvPath, entities);
        }
        catch (IOException ex) when ((ex.HResult & 0xFFFF) is 32 or 33) // ERROR_SHARING_VIOLATION / ERROR_LOCK_VIOLATION
        {
            if (!Utils.TryFocusOpenWriter())
            {
                Utils.MsgTaskDlg(Handle, "Datenquelle in Verwendung",
                    "»Serienbrief.csv« ist noch geöffnet (z. B. in LibreOffice Writer oder Word).\n" +
                    "Bitte schließe die Datenquelle bzw. das Dokument und versuche es erneut.",
                    TaskDialogIcon.ShieldWarningYellowBar);
            }
            return;
        }
        ...
        if (useWord == true) { WordSerienbrief(csvPath, count); }
        else { LibreSerienbrief(csvPath, count); }
    }

    private void WordSerienbrief(string csvPath, int count)
    {
        // Hauptdokument wählen. Wird der Dialog abgebrochen (kein Dokument gewählt), bleibt templatePath leer
        // → in WordManager.ConnectMailMergeDataSource wird dann ein neues, leeres Word-Dokument angelegt.
        openFileDialog.Title = "Hauptdokument wählen (Abbrechen = neues Dokument)";
        openFileDialog.Filter = "Word-Dokumente|*.docx;*.docm;*.doc;*.dotx;*.dotm;*.dot|Alle Dateien (*.*)|*.*";
        openFileDialog.Multiselect = false;
        openFileDialog.CheckFileExists = true;
        var last = _settings.MailMergeTemplatePath;
        if (!string.IsNullOrEmpty(last) && File.Exists(last))
        {
            openFileDialog.InitialDirectory = Path.GetDirectoryName(last) ?? string.Empty;
            openFileDialog.FileName = Path.GetFileName(last);
        }
        else { openFileDialog.FileName = string.Empty; }

        var templatePath = string.Empty; // leer = neues Dokument
        if (openFileDialog.ShowDialog(this) == DialogResult.OK)
        {
            templatePath = openFileDialog.FileName;
            _settings.MailMergeTemplatePath = templatePath;
            SettingsManager.Save(_settings, _settingsPath);
        }

        if (_settings.ShowWordMailMergeHint && !ShowWordMailMergeHint(templatePath, count)) { return; } // im Hinweis abgebrochen
        toolStripStatusLabel.Text = $"Serienbrief: {count} Adressen als Datenquelle bereitgestellt.";
        WordManager.ConnectMailMergeDataSource(templatePath, csvPath, Handle);
    }

    private bool ShowWordMailMergeHint(string templatePath, int count)  // wird nur aufgerufen, wenn ShowMailMergeHint=true; liefert true, wenn fortgefahren werden soll
    {
        var btnContinue = new TaskDialogButton("&Weiter – Word öffnen"); // TaskDialogButton.Continue hat den Nachteil, dass er rechts von Abbrechen steht
        var btnCancel = TaskDialogButton.Cancel;
        using var icon = new TaskDialogIcon(Resources.Word32);
        var verification = new TaskDialogVerificationCheckBox { Text = "Immer anzeigen", Checked = true };
        var page = new TaskDialogPage
        {
            Caption = Application.ProductName,
            Heading = "Serienbrief mit Microsoft Word",
            Text =
                $"{count} Adressen werden als Datenquelle mit {(string.IsNullOrEmpty(templatePath) ? "einem neuen Haupt-\ndokument" : $"dem Hauptdokument\n»{Path.GetFileName(templatePath)}«")} verbunden.\n\n" +
                "Ein Word-Serienbrief nutzt Seriendruckfelder, keine Textmarken!\n" +
                "Klicke auf »Seriendruckfeld einfügen« um die Felder einzufügen.\n\n" +
                "Tipp: »Empfängerliste bearbeiten« erlaubt die Adressenauswahl.",
            Icon = icon,
            AllowCancel = true,
            SizeToContent = true,
            Buttons = { btnContinue, btnCancel },
        ...
        verification.CheckedChanged += (s, e) =>
        {
            ...
            {
                Utils.MsgTaskDlg(page.BoundDialog?.Handle ?? Handle, "Serienbrief mit Microsoft Word",
                    "Du kannst diesen Hinweis in den Einstellungen\n(Abfragen ▸ Serienbrief) wieder einschalten.", new(Resources.info32));
            }
        };
        var result = TaskDialog.ShowDialog(Handle, page);
        if (result != btnCancel && !verification.Checked)
        {
            _settings.ShowWordMailMergeHint = false; // dauerhaft ausblenden
            SettingsManager.Save(_settings, _settingsPath);
        }
        return result == btnContinue;
    }

    private void LibreSerienbrief(string csvPath, int count)
    {
        var folder = Path.GetDirectoryName(csvPath) ?? string.Empty;
        var odbPath = Path.Combine(folder, MailMergeManager.OdbFileName);
        try { MailMergeManager.WriteOdb(odbPath, folder); }  // ODB-Datei enthält die CSV-Datei als Datenquelle
        catch (IOException ex) when ((ex.HResult & 0xFFFF) is 32 or 33) // gesperrt (in Verwendung)
        {
            if (!Utils.TryFocusOpenWriter())
            {
                Utils.MsgTaskDlg(Handle, "Datenquelle in Verwendung",
                    $"Die Datei »{MailMergeManager.OdbFileName}« ist noch geöffnet.\nBitte schließe sie und versuche es erneut.",
                    TaskDialogIcon.ShieldWarningYellowBar);
            }
            return;
        }
        catch (Exception ex) { Utils.ErrTaskDlg(Handle, ex); odbPath = string.Empty; }
        using var writerIcon = new TaskDialogIcon(Resources.mergeemail32);
        var btnStartWriter = new TaskDialogButton("&Writer starten") { AllowCloseDialog = false };
        var btnOpenFolder = new TaskDialogButton("&Ordner öffnen") { AllowCloseDialog = false };
        var btnClose = TaskDialogButton.Close;
        var page = new TaskDialogPage
        {
            Icon = writerIcon is not null ? writerIcon : null,
            Caption = "Seriendruck-Hinweis",
            Heading = "Serienbrief-Datenquelle für LibreOffice Writer",
            Text =
                $"{count} Adressen wurden in der Datenbankdatei gespeichert:\n{odbPath}\n\n" +
                "Starte im Writer-Menü »Extras« den »Seriendruck-Assistent«.\n" +
                "Klicke auf »Datenbank austauschen« oder »Adressliste aus-\n" +
                "wählen«, wähle »Serienbrief.odb« aus und dann »Adressen«.\n\n" +
                "Tipps: Drücke Strg+Umschalt+F4, um die Datenquelle anzu-\n" +
                "zeigen. Nun kannst du beliebig Spaltenköpfe der benötigten\n" +
                "Felder mit der Maus in das Augangsdokument ziehen.\n" +
                "Auf das Angebot eine Briefanrede zu erstellen, solltest du ver-\n" +
                "zichten und stattdessen das Feld »Grussformel« verwenden.",
            AllowCancel = true,
            SizeToContent = true,
            Buttons = { btnStartWriter, btnOpenFolder, btnClose },
        };
        btnStartWriter.Click += (s, e) => StartLibreOfficeWriter();
        ...
        page.Created += (s, e) =>
        {
            var hDlg = page.BoundDialog?.Handle ?? IntPtr.Zero;
            if (hDlg != IntPtr.Zero)  // Da der Dialog ohne Besitzer (nicht-modal) angezeigt wird: beim Erstellen auf "immer im Vordergrund"
            {
                NativeMethods.SetWindowPos(hDlg, NativeMethods.HWND_TOPMOST, 0, 0, 0, 0, NativeMethods.SWP_NOSIZE | NativeMethods.SWP_NOMOVE | NativeMethods.SWP_NOACTIVATE);
            }
        };
        TaskDialog.ShowDialog(page);
    }

    private void StartLibreOfficeWriter()
    {
        try
        {
            var writerDoc = NativeMethods.GetLastVisibleHandleByTitleEnd("– LibreOffice Writer");
            if (writerDoc != IntPtr.Zero) { NativeMethods.SetForegroundWindow(writerDoc); return; }
            var anyLibre = NativeMethods.GetLastVisibleHandleByTitleEnd("LibreOffice");
            if (anyLibre != IntPtr.Zero) { NativeMethods.SetForegroundWindow(anyLibre); return; }

            using var key = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\LibreOffice\UNO\InstallPath");
            var libreOfficeDir = key?.GetValue(null) as string;
            if (!string.IsNullOrEmpty(libreOfficeDir))
            {
                var exePath = Path.Combine(libreOfficeDir, "soffice.exe");
                if (File.Exists(exePath)) { Process.Start(new ProcessStartInfo { FileName = exePath, Arguments = "--writer", UseShellExecute = true }); }
                else { Utils.MsgTaskDlg(Handle, "soffice.exe wurde nicht gefunden", exePath); }
            }
            else { Utils.MsgTaskDlg(Handle, "LibreOffice-Installationspfad nicht gefunden.", @"Computer\HKEY_LOCAL_MACHINE\SOFTWARE\LibreOffice\UNO\InstallPath"); }
        }
    ...
    {  //Alle Werte einmalig sauber auslesen und trimmen; das verbessert die Lesbarkeit und Performance
        ...
        var strasse = tbStrasse.Text.Trim() ?? string.Empty;
        ...
        var anredeAkkusativ = string.Equals(anrede, "Herr", StringComparison.OrdinalIgnoreCase) ? "Herrn" : anrede;
        var empfaenger = string.Join(" ", new[] { anredeAkkusativ, praefix, vorname, nachname }.Where(static s => !string.IsNullOrWhiteSpace(s)));
        var namen = string.Join(" ", new[] { praefix, vorname, zwischenInitial, nachname, suffix }.Where(static s => !string.IsNullOrWhiteSpace(s)));
        var pfPLZ = string.IsNullOrEmpty(plz) ? postfach : plz;
        bookmarkTextDictionary["NAMEN"] = string.IsNullOrEmpty(nachname) && !string.IsNullOrEmpty(firma) ? firma : namen;
        bookmarkTextDictionary["EMPFAENGER"] = string.IsNullOrEmpty(nachname) && !string.IsNullOrEmpty(firma) ? firma : empfaenger;
        bookmarkTextDictionary["ANSCHRIFT"] =
            string.Join("\v", new[] { empfaenger, firma, strasse, $"{pfPLZ} {ort}".Trim() }.Where(static s => !string.IsNullOrWhiteSpace(s)));
        ...
        bookmarkTextDictionary["Adresse"] = strasse; // Schlüssel "Adresse" = Straße + Hausnummer (analog Google/Word)
    ...
    private async void WordHelpToolStripMenuItem_Click(object sender, EventArgs e)
    {
        FillWordProcessingDictionary();
        if (WordManager.ShowWordBookmarksInfoDialog(Handle, [.. bookmarkTextDictionary.Keys], _settings.WordProcessorProgram)) { await OpenSettingsAsync(); }  // Link angeklickt, Einstellungen öffnen
    }
    ...
    #region Drucken, Umschlag, vCard, E-Mail und Zwischenablage
    ...
    #region Geburtstage und Erinnerungen
    ...
    #region Fritz!Box-Anrufmonitor

    private void InitFritzMonitor()
    {
        if (_fritzMonitor is not null)  // Events zuerst abmelden – verhindert, dass der alte Monitor nach dem Dispose noch verzögert ConnectionChanged(false) feuert
        {
        ...
        tsBtnFritzMonitor.Visible = false;  // UI sofort zurücksetzen – unabhängig vom Hintergrund-Thread
        tsBtnFritzMonitor.Image = Resources.telephoneOff16;
    ...
    private void FritzMonitor_CallEventReceived(object? sender, FritzCallEvent evt)
    {
        ...
        if (evt.Type != FritzCallType.Ring || evt.IsAnonymous) { return; }  // Nur bei eingehenden Anrufen mit bekannter Nummer reagieren

        if (_fritzCalledNumberFilter.Count > 0) { if (!_fritzCalledNumberFilter.Contains(evt.CalledNumber)) { return; } }  // MSN-Filter
        _recentCalls.Insert(0, evt);  // Liste für TasksDialog aufbauen 
        if (_recentCalls.Count > 10) { _recentCalls.RemoveAt(10); }  // max. 10 Einträge
        if (!Visible) { RestoreFromTray(); }
        else if (WindowState == FormWindowState.Minimized) { NativeMethods.ShowWindow(Handle, NativeMethods.SW_RESTORE); }
        Activate();
        ...
        tsBtnFritzMonitor.Text = $"{evt.CallerNumber} ({DateTime.Now:HH:mm:ss}, {evt.CalledNumber})";
    ...
    private async void TsBtnFritzMonitor_Click(object sender, EventArgs e)
    {
        ...
        if (string.IsNullOrEmpty(normalized))
        {
            ...
            if (_recentCalls.Count == 0) { textBuilder.AppendLine("Keine eingehenden Anrufe seit Programmstart registriert."); }
            else
            {
                textBuilder.AppendLine("Letzte Anrufe:");
                foreach (var call in _recentCalls) { textBuilder.AppendLine($"• {call.Timestamp:HH:mm:ss} Uhr  —  {call.CallerNumber} (an {call.CalledNumber})"); }
            }
        ...
        var contactsFirst = _settings.FritzContactsFirst && _allGoogleContacts != null;  // Primären Suchbereich auswählen (Contacts nur, wenn auch geladen)
        ...
        if (contactsFirst && contactBSource.Count == 0)  // Kein Treffer im Kontakte-Tab → Adressen als Fallback
        {
            SwitchTab(addressTabPage);
            ApplyGlobalSearch(normalized);
        }
        tsBtnFritzMonitor.Tag = null;  // Tag zurücksetzen, damit bei nächsten Klick nicht wieder die gleiche Nummer gesucht wird   
    }
    ...
    #region Infobereich (Tray), Hotkey und Fensterzustand
    ...
    #region Kontextmenüs und Menüsteuerung
    ...
    #region Hilfe, Update und Programmende
    ...
    private async void FrmAdressen_FormClosing(object sender, FormClosingEventArgs e)
    {
        ...
        _googleCts?.Cancel();
        detailTimer.Stop();  // kein verzögertes Detail-Laden mehr während des Schließens
    ...
    protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
    {
        switch (keyData)
        {
            ...
            case Keys.Enter:
                {
                    ...
                    if (activeCtrl is TextBox tbb && NativeMethods.IsAutoCompleteDropdownOpen())
                    {
                        var aktuellerText = tbb.Text;  // Dropdown sichtbar: Vorschlag übernehmen, Groß-/Kleinschreibung normalisieren, NICHT weiternavigieren
                        ...
                        NativeMethods.HideAutoCompleteDropdown();  // Dropdown schließen 
                        tbb.SelectionStart = tbb.Text.Length;
                        tbb.SelectionLength = 0;  // Cursor ans Ende – nach dem Toggle
                        return true;
                    }
                    break;
                }
            ...
            case Keys.F11:
                // ProcessCmdKey kann nicht async sein → bewusstes "Fire & Forget" wie an anderen Stellen
                if (WordManager.ShowWordBookmarksInfoDialog(Handle, [.. bookmarkTextDictionary.Keys], _settings.WordProcessorProgram)) { _ = OpenSettingsAsync(); }
            ...
            case Keys.F3:  // Weitersuchen
                ...
                if (focusedCtrl is TextBox { Multiline: true } tb) { _searchManager.FindNext(tb); }
                else if (tabulation.SelectedTab == tabPageDoku) { searchTextBox.Focus(); }
                else { searchTSTextBox.TextBox.Focus(); }
                return true;
            case Keys.F9:
                if ((tabControl.SelectedTab == addressTabPage && addressDGV.Rows.Count > 0) || (tabControl.SelectedTab == contactTabPage && contactDGV.Rows.Count > 0))
                {
            ...
            case Keys.H | Keys.Control:
                if (min2TrayTSButton.Visible) { HideToTray(); }
                return true;
            case Keys.OemMinus | Keys.Control:
            case Keys.Subtract | Keys.Control:
                HideToolStripMenuItem_Click(this, EventArgs.Empty);
                return true;
        }
    ...
    private void PWAToolStripMenuItem_Click(object sender, EventArgs e) => Utils.StartLink(Handle, "https://app.schielen.de/");
    ...
    private async void UpdateCheckToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        var btnSkip = new TaskDialogButton("Überspringen") { AllowCloseDialog = false };  // Verhindert das Schließen des Dialogs
