...
public partial class FrmAdressen : Form
{
    ...
    {
        // Sicherheitscheck: Nur prüfen, wenn wir im Kontakt-Tab sind
        if (tabControl.SelectedTab == contactTabPage && contactBindingSource.Current is Contact currentContact)
        {
            ...
            if (string.IsNullOrEmpty(currentContact.ResourceName))
            {
                ...
                {
                    // Menü-Öffnen abbrechen
                    e.Cancel = true;

                    await CreateContactAsync();

                    // Hier könnten Sie das Menü theoretisch auch wieder öffnen, 
                    // falls nach dem Erstellen sofort weitergemacht werden soll.
                    return;
                }
            }
            ...
            if (ContactChanges_Check())
            {
                // Abbrechen, damit das Menü nicht über dem Dialog liegt
                e.Cancel = true;

                await AskSaveContactChangesAsync();

                // Prüfen, ob Änderungen jetzt bereinigt sind
                if (!ContactChanges_Check())
                {
    ...
    private void UpdateSaveButton()
    {
        ...
        var hasGoogleChanges = ContactChanges_Check() || contactNewRowIndex >= 0;
        contactTabPage.Text = hasGoogleChanges ? "Google Kontakte*" : "Google Kontakte";
        if (tabControl.SelectedTab == addressTabPage) { saveTSButton.Enabled = hasSqlChanges; }
        else if (tabControl.SelectedTab == contactTabPage) { saveTSButton.Enabled = hasGoogleChanges; }
    }
    ...
    private async void UpdateCheckToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        var updatePage = new TaskDialogPage()
        {
            ...
            Caption = appName,
            Heading = "Automatische Updatesuche",
            Text = "Wie häufig soll nach einem Update gesucht werden?",
        ...
        progressPage.Created += async (s, args) =>
        {
            ...
            if (latestVersion != null)
            {
                ...
                else
                {
                    ...
                    footText = $""" 
                    {spacer}Status: {(latestVersion > currentVersion ? "Update verfügbar" : "Aktuell")}
                    {spacer}Installiert: {currentVersion.ToString(3)}
                    {spacer}Verfügbar: {latestVersion.ToString(3)}
                    {spacer}Datum: {releaseDate}
                    """;
                }
            }
            else { footText = "Der Update-Server konnte nicht erreicht werden."; }
            updatePage.Footnote = new TaskDialogFootnote(footText);
            if (_settings.UpdateIndex == 1) { rbn1.Checked = true; }  // RefreshUpdateUI hat möglicherweise den Index geändert
            else if (_settings.UpdateIndex == 2) { rbn2.Checked = true; }
            else if (_settings.UpdateIndex == 3) { rbn3.Checked = true; }
            else { rbn0.Checked = true; }  // Standardmäßig "Jeden Tag"
            progressPage.Navigate(updatePage);  // Zur Einstellungsseite navigieren
        };
        if (TaskDialog.ShowDialog(this, progressPage) == TaskDialogButton.OK)
        {
